      ...
      if (appletId != THIS_APPLET_ID && appletId != this.weServices.appletId) {
        ...
        return html`
            <div style="display:flex; flex-direction:column; gap:10px; padding:7px;">
                <div style="background: blue; margin: auto;">${appletInfo.appletName}</div>
            </div>
        `;
      }
      ...
      let subjectItems = Object.entries(appletThreads).map(([subjectHash, ppAhs]) => {
        ...
        return html`
          ...
          if (subjectItems.length == 0) {
            return html`
                  <div style="display:flex; flex-direction:column; gap:10px; padding:7px;">
                    <div style="color: gray; margin: auto;">${msg('No subjects found')}</div>
                  </div>
              `;
          }
          /** render subjectItems */
          return html`${subjectItems}`
