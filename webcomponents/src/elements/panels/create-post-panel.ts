...
export class CreatePostPanel extends DnaElement<unknown, ThreadsDvm> {
  ...
  private async beforeCreate(): Promise<[ActionId, boolean]> {
    ...
    if (!mainThreads || mainThreads.length == 0) {
      ...
      const [_ts, ppAh] = await this._dvm.threadsZvm.publishThreadFromSemanticTopic(appletId, MAIN_TOPIC_ID, "main", defaultLimitations(), defaultModeration());
      mainThreadAh = ppAh;
  ...
  /** */
  private async afterCreate(beadAh: ActionId, beadType: BeadType, createdMainThread: boolean) {
    /** Create comment thread and  get notifications for your own post */
    const subject: Subject = {
      address: beadAh.b64,
      name: "_unknown_",
      typeName: beadType,
      dnaHashB64: this._dvm.cell.address.dnaId.b64,
  ...
  async onCreateText() {
    ...
    inputElem.value = "";
    /** After */
    this.afterCreate(beadAh, ThreadsEntryType.TextBead, createdMainThread)
  }
