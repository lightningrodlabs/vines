...
import {DM_SUBJECT_TYPE_NAME, GLOBAL_TIME_INDEX, PP_ITEM_TYPE, ROOT_ANCHOR_SEMANTIC_TOPICS, ROOT_ANCHOR_SUBJECTS, SEMANTIC_TOPIC_TYPE_NAME, THREADS_DEFAULT_COORDINATOR_ZOME_NAME, THREADS_DEFAULT_INTEGRITY_ZOME_NAME, VINES_DEFAULT_ROLE_NAME, BaseBeadKind, DirectGossipProtocol, EntryStateChange, NotifiableEvent, NotifySetting, SystemSignalProtocol, ThreadsEntry, ThreadsEntryKind, ThreadsLinkType, ThreadsSignalProtocol, AddAnyBeadInput, AddEncBeadInput, AddEntryAsBeadInput, AddEntryBeadInput, AddManyTextBeadAtInput, AddReactionInput, AddTextAtAndNotifyInput, AddTextBeadAtInput, AnyBead, Bead, BeadLink, BroadcastGossipInput, CommitGlobalLogInput, DecryptBeadInput, EncryptBeadInput, EncryptedBead, EntryBead, EntryInfo, GetDataTypeInput, GetLatestBeadsInput, GetProtocolsInput, GlobalLastProbeLog, GossipSignal, NotifyPeerInput, ParticipationProtocol, ProbeAllOutput, SemanticTopic, SendInboxItemInput, SetNotifySettingInput, Subject, SystemSignal, TextBead, ThreadLastProbeLog, ThreadsNotification, ThreadsProperties, ThreadsSignal, UpdateTopicInput, WeaveNotification, } from './threads.types';
import {
...
export class ThreadsProxy extends ZomeProxy {
  ...
  async broadcastGossip(input: BroadcastGossipInput): Promise<void> {
    return this.call('broadcast_gossip', input);
  }

  async emitNotification(input: NotifyPeerInput): Promise<void> {
    return this.call('emit_notification', input);
  }
