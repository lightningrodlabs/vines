...
import {THREADS_ZOME_NAME, ThreadsEntry, ThreadsLinkType, TopicType, GetProtocolsInput, TypedAnchor, LeafLink, Bead, TextMessage, SemanticTopic, ParticipationProtocol, GlobalQueryLog, ThreadQueryLog, } from './threads.types';
import {
/** types.ts */
HoloHash,
AgentPubKey,
...
export class ThreadsProxy extends ZomeProxy {
  ...
  async getAnchorChildren(anchor: string): Promise<TypedAnchor[]> {
    return this.call('get_anchor_children', anchor);
  }

  async getAnchorLinks(ta: TypedAnchor): Promise<LeafLink[]> {
    return this.call('get_anchor_links', ta);
  }
