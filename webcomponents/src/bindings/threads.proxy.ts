...
// @ts-ignore
import {DM_SUBJECT_TYPE_NAME, GLOBAL_TIME_INDEX, PP_ITEM_TYPE, ROOT_ANCHOR_SEMANTIC_TOPICS, ROOT_ANCHOR_SUBJECTS, SEMANTIC_TOPIC_TYPE_NAME, THREADS_DEFAULT_COORDINATOR_ZOME_NAME, THREADS_DEFAULT_INTEGRITY_ZOME_NAME, VINES_DEFAULT_ROLE_NAME, BaseBeadKind, NotifySetting, Rules, ThreadsEntry, AddAnyBeadInput, AddEncBeadInput, AddEntryAsBeadInput, AddEntryBeadInput, AddManyTextBeadAtInput, AddTextBeadAtInput, AnyBead, AppStringTip, AutoRules, BanAgentInput, Bead, BeadLink, CommitGlobalLogInput, DecryptBeadInput, EncryptBeadInput, EncryptedBead, EntryBead, FileRules, FindSubjectsInput, GetDataTypeInput, GetLatestBeadsInput, GlobalLastProbeLog, ManualRules, NotifyPeerInput, ParticipationProtocol, ProbeAllOutput, PublishDmThreadInput, PublishTopicInput, ReactionInput, SemanticTopic, SetNotifySettingInput, Subject, TextBead, TextRules, ThreadLastProbeLog, ThreadsProperties, UpdatePpTitleInput, UpdateTopicInput, } from './threads.types';
// @ts-ignore
import {
// @ts-ignore
...
export class ThreadsProxy extends ZomeProxy {
  ...
  async getPpTitle(ppAh: ActionArray): Promise<string> {
    return this.callBlocking('get_pp_title', ppAh);
  }
  ...
  async probeAllFlagged(ppAh: ActionArray): Promise<void> {
    return this.call('probe_all_flagged', ppAh);
  }

  async probeAllBanned(ppAh: ActionArray): Promise<void> {
    return this.call('probe_all_banned', ppAh);
  }
