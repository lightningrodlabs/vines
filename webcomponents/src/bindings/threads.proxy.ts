...
import {GLOBAL_TIME_INDEX, PP_ITEM_TYPE, ROOT_ANCHOR_SEMANTIC_TOPICS, ROOT_ANCHOR_SUBJECTS, SEMANTIC_TOPIC_TYPE_NAME, THREADS_DEFAULT_COORDINATOR_ZOME_NAME, THREADS_DEFAULT_INTEGRITY_ZOME_NAME, THREADS_DEFAULT_ROLE_NAME, DirectGossip, NotifiableEvent, SignalPayload, ThreadsEntry, ThreadsLinkType, AddEntryAsBead, AddManyTextMessageAtInput, AddReactionInput, AddTextAndMentionsAtInput, AddTextMessageAtInput, AddTextWithMentionsInput, AnyBead, Bead, BeadLink, CreatePpInput, EntryBead, GetLatestBeadsInput, GetProtocolsInput, GlobalLastProbeLog, NotifyPeerInput, ParticipationProtocol, ProbeAllOutput, SemanticTopic, SendInboxItemInput, SignalPeersInput, Subject, TextBead, ThreadLastProbeLog, WeaveNotification, WeaveSignal, } from './threads.types';
import {
/** types.ts */
HoloHash,
AgentPubKey,
...
export class ThreadsProxy extends ZomeProxy {
  ...
  async addTextMessageAtWithMentions(input: AddTextAndMentionsAtInput): Promise<[ActionHash, string, [AgentPubKey, WeaveNotification][]]> {
    return this.call('add_text_message_at_with_mentions', input);
  }

  async queryTextMessages(): Promise<[Timestamp, ActionHash, TextBead][]> {
    return this.call('query_text_messages', null);
  }

  async getTextMessage(ah: ActionHash): Promise<[Timestamp, AgentPubKey, TextBead]> {
    return this.call('get_text_message', ah);
  }

  async getManyTextMessage(ahs: ActionHash[]): Promise<[Timestamp, AgentPubKey, TextBead][]> {
    return this.call('get_many_text_message', ahs);
  }

  async addTextMessage(texto: TextBead): Promise<[ActionHash, string, Timestamp]> {
  ...
  async createParticipationProtocol(input: CreatePpInput): Promise<[ActionHash, Timestamp, [AgentPubKey, WeaveNotification] | null]> {
