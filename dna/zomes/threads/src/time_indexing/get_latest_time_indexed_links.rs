  ...
  while res.len() < target_count && current_search_tp.as_ref().len() >= root_tp.as_ref().len() {
    ...
    if current_search_tp.exists()? {
      ...
      let mut older_children_pairs: Vec<(Path, i32, Link)> = children
        ...
        .map(|(c, l)| {
          let tuple = (current_search_tp.path.clone(), convert_component_to_i32(&c)?, l);
          Ok(tuple)
        })
      ...
      debug!("Starting search of descendants of {} (depth {})", path2anchor(&current_search_tp.path).unwrap(), depth);
      ...
      debug!("Finished searching descendants of {} (depth {}). Raw children {}. leafs added {}"
        , path2anchor(&current_search_tp.path).unwrap(), depth, raw_children_dbg_info, links_added.len());
    }
...
fn append_target_links_recursive(
  ...
  /// Traverse tree
  for (parent_path, compi32, child_link) in children {
    if depth == 0 {
      ///
      //let final_path = parent_path.clone().append_component( Component::from(format!("{}", compi32)));
      /// Grab all children at the hour level
      let links = get_links(
        child_link.target,
        //final_path.path_entry_hash()?,
        LinkTypeFilter::single_type(link_type.zome_index, link_type.zome_type),
        link_tag.clone(),
      )?;
      debug!("  get_links() of parent {} + {} : {} found", path2anchor(&parent_path).unwrap(), compi32, links.len());
      /// Form leaf path
      let mut pairs = links
        .into_iter()
        .map(|l| {
      ...
      debug!("  get_links(grandchildren) of parent {} + {} : {} found", path2anchor(&parent_path).unwrap(), compi32, grandchildren.len());
      ...
      let grandchildren_pairs = grandchildren
        ...
        .map(|(c, l)| {
          ...
          leaf_path.append_component(comp.clone());
          let tuple = (leaf_path, convert_component_to_i32(&c)? /* compi32*/, l);
          Ok(tuple)
        })
        .collect::<Result<Vec<_>, WasmError>>()?;
      /// Go deeper
