...
///
#[hdk_extern]
pub fn get_thread_log(eh: EntryHash) -> ExternResult<ThreadLastSearchLog> {
...
/// Return ActionHash
#[hdk_extern]
pub fn commit_thread_log(tql: ThreadLastSearchLog) -> ExternResult<ActionHash> {
  let ah = create_entry(ThreadsEntry::ThreadQueryLog(tql.clone()))?;
  Ok(ah)
}
...
#[hdk_extern]
pub fn query_thread_logs(_: ()) -> ExternResult<Vec<ThreadLastSearchLog>> {
  let entry_type = EntryType::App(ThreadsEntryTypes::ThreadQueryLog.try_into().unwrap());

  /// Get Create actions
  let create_records = get_all_typed_local::<ThreadLastSearchLog>(entry_type.clone())?;
  ...
  for (record) in updates {
    ...
    let updated_log: ThreadLastSearchLog = get_typed_from_record(record)?;
