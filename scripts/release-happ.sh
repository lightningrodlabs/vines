...
#   1. `artifacts/vines.happ` from a local `npm run build:happ:release` — used when
...
if [ $# -ge 1 ]; then
  ...
  happ="$tmp/wa/vines.happ"
else
  happ="artifacts/vines.happ"
  if [ ! -f "$happ" ]; then
...
if [ "$got" != "$EXPECTED_SHA" ]; then
  ...
  echo "package.json, update .happ-version, write $got into .happ-sha256," >&2
  echo "and re-run. Everyone on the old DNA stays on the old network." >&2
  exit 1
fi
echo "Verified canonical happ sha256 = $got"

notes="Frozen canonical vines.happ reused by every webhapp release to keep all installs on the same network. DNA sha256: $EXPECTED_SHA. Do NOT rebuild."
...
echo "Done. release-webhapp.yaml will download vines.happ from $HAPP_TAG."
