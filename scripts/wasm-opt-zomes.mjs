 ...
 *     installing vines from a release joins that network.
 ...
 *
 * Which zomes exist is read from the DNA manifests rather than guessed from
 * file names, so a zome added later is covered automatically. The happ has two
 * DNAs: vines' own (threads) and the files DNA built from submodules/files.
 *
...
const MANIFESTS = ['dna/workdir/dna.yaml', 'submodules/files/dna/workdir/dna.yaml'];

const zomes = MANIFESTS.flatMap((MANIFEST) => {
