...
export class VinesPage extends DnaElement<ThreadsDnaPerspective, ThreadsDvm> {
  ...
  onImportFile = (data: ImportData) => {
    console.log("onImportFile()", this, data);
    const summary = this.shadowRoot!.getElementById("importSummary") as ImportSummaryDialog;
...
//    const groupProfile = determinerGroupProfile(this._dvm.dnaProperties, [this.weServices, 0]);
    ...
    return html`
        <div id="mainDiv"
            ...
            <export-summary-dialog id="export-dialog"
                                   @export-confirmed=${async (e: CustomEvent<Set<string>>) => {
                                     const content = this._dvm.exportPerspective(APP_VERSION, e.detail);
  ...
  async onSettingsMenu(e: any): Promise<void> {
    ...
    switch (e.detail.item.id) {
      ...
      // @ts-ignore
      case "exportAllItem":
        if (content == "") content = await this._dvm.exportAllPerspective(APP_VERSION);
      case "exportItem":
